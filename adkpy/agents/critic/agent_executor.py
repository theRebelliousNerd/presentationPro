...
class CriticAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        logger.debug('[CriticAgentExecutor] execute exiting')

    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        session_id = context.context_id
        if session_id in self._active_sessions:
            logger.info(
                f'Cancellation requested for active critic session: {session_id}'
            )
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f'Cancellation requested for inactive critic session: {session_id}'
            )
