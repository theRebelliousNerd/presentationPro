...
class DesignAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        logger.debug('[DesignAgentExecutor] execute exiting')

    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        session_id = context.context_id
        if session_id in self._active_sessions:
            logger.info(
                f'Cancellation requested for active design session: {session_id}'
            )
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f'Cancellation requested for inactive design session: {session_id}'
            )
