...
const defaultTools = [
  ...
  { name: "brand.detect_logo", path: "/v1/visioncv/logo", sample: { target_image_b64: "data:image/png;base64,...", reference_logo_b64: "data:image/png;base64,..." } },
...
export default function VisionCVDev() {
  ...
  const updateBodyWithPrimary = (dataUrl: string) => {
    try {
      ...
      if (name.includes('ocr') || name.includes('bar_chart') || name.includes('line_graph') || name.includes('brand_colors')) {
  ...
  const onDropFiles = async (files: FileList | File[]) => {
    ...
    // Secondary (optional, for logo reference)
    if (imgs.length > 1 && tool.name.includes('detect_logo')) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          >
            ...
            <div className="text-xs text-muted-foreground mb-2">Primary screenshot required{tool.name.includes('detect_logo') ? ", optional second image for logo reference" : ''}.</div>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
              multiple={tool.name.includes('detect_logo')}
              className="hidden"
            ...
            <div className="grid grid-cols-2 gap-3 w-full mt-2">
              ...
              {tool.name.includes('detect_logo') && (
                <div className="flex flex-col items-center gap-2">
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                onClick={(e)=>{ e.stopPropagation(); setPrimaryPreview(null); setSecondaryPreview(null); setBody(JSON.stringify(tool.sample, null, 2)); }}
              >Clear</button>
              {result && result !== '{}' && (
                <button
                  className="border rounded px-2 py-1 text-xs"
